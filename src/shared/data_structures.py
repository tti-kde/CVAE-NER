...
import numpy as np
from shared.bc8_data_reshape import json2input
from shared.big_bio_loader import bigbio2input
import random
...
class Dataset:
    def __init__(
        ...
        if bool_bigbio:
            self.js = bigbio2input(json_file)
        else:
            self.bool_bc8 = bool_bc8
            self.js = self._read(json_file, pred_file)
            if self.bool_bc8:
                self.input_js = self.js
                self.js = json2input(self.js[0]["documents"])
        if doc_range is not None:
            random.shuffle(self.js)
            self.js = self.js[doc_range[0] : doc_range[1]]
        self.documents = [Document(js, bool_bc8, args=args) for js in self.js]

    def update_from_js(self, js, args=None):
        self.js = js
        self.documents = [Document(js, args=args) for js in self.js]

    def _read(self, json_file, pred_file=None):
        if self.bool_bc8:
            gold_docs = [json.load(open(json_file))]
        else:
...
class Document:
    def __init__(self, js, bool_bc8, args=None):
        ...
        self.sentences = [
            Sentence(entry, sentence_start, sentence_ix, bool_bc8, args=args)
...
class Sentence:
    def __init__(self, entry, sentence_start, sentence_ix, bool_bc8, args=None):
        self.sentence_start = sentence_start
        self.text = entry["sentences"]
        if bool_bc8:
            self.word_id2offset = entry["word_id2offset"]
        self.sentence_ix = sentence_ix
        # Gold
        if "ner_flavor" in entry:
            self.ner = [
                NER(this_ner, self.text, sentence_start, flavor=this_flavor, args=args)
                for this_ner, this_flavor in zip(entry["ner"], entry["ner_flavor"])
            ]
        elif "ner" in entry:
            self.ner = [
                NER(this_ner, self.text, sentence_start, args=args) for this_ner in entry["ner"]
            ]
        ...
        if "predicted_ner" in entry:
            self.predicted_ner = [
                NER(this_ner, self.text, sentence_start, flavor=None, args=args)
...
class NER:
    def __init__(self, ner, text, sentence_start, flavor=None, args=None):
        self.span = Span(ner[0], ner[1], text, sentence_start)
        if args.case_tsne is not None:
            ne_text = text[ner[0] - sentence_start : ner[1] - sentence_start + 1]
            for i, case_ne in enumerate(args.case_tsne):
                if case_ne in ne_text:
                    self.label = 500 + i
                    break
            else:
                self.label = ner[2]
